...
filepath = 'E:/PYTEST/A/rlData.csv'
...
# 保持原有数据处理逻辑不变，直接替换Plotly相关代码
import plotly.express as px
import plotly.graph_objects as go

fig = go.Figure()
# 问题1：修正冗余的 go.Scatter 嵌套（原代码写了 go.Scatter(go.Scatter(...))，多余内层嵌套）
...
fig.update_layout(
    ...
    yaxis=dict(
        # 问题2：修正 titlefont 错误（核心修改！）
        # 原代码直接写 titlefont=dict(...)，无效！需嵌套在 title=dict(...) 中
        title=dict(
            text='Close (USD)',  # Y轴标题文本
            font=dict(  # 用 title.font 替代原有的 titlefont
                family='Rockwell',
                size=12,
                color='white',
            )
